...
class MoCo(nn.Module):
    ...
    def forward_aug_nn(self, x1, x2): # 带增强邻居的 forward
        q = self.encoder_q(x1)
        latent = self.encoder_q.get_embedding(x1)
        q = F.normalize(q, dim=1)

        c = x2.size(0) // x1.size(0)
        qc = q.unsqueeze(1) # [B,1,D]
        for _ in range(1, c):  # [B, c, D]
            qc = torch.cat([qc, q.unsqueeze(1)], dim=1)
        qc = qc.reshape(-1, q.size(1)) # [B*c, D]
        ...
        pos_sim2 = pos_sim2.reshape(-1, c) # [B, c]
        ...
        penalty = self.alpha * (torch.mean(torch.abs(latent))) # 鼓励 embedding 变稀疏 / 更小
        loss += penalty 
...
class Network(nn.Module):
    def __init__(self, 
        ...
        for v in range(view):
            ...
            self.decoders.append(adjDecoder(self.dropout, act=lambda x: x)) 
    ...
    def forward(self, adjs_n, g_feats):
        ...
        x_joint,atten_score = self.atten_cross(x0, x1)
        feats = [x0, x1, x_joint]
