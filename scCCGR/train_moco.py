...
def pretrain_epoch(model, 
    ...
    for step, pre_index in enumerate(range(data[0].shape[0] // batch_size + 1)):
        ...
        loss, loss1, loss2 = model(x1 = x[0], x2 = x[1], x1_aug = x_nei[0], x2_aug = x_nei[1])
...
def main():
    ...
    parser.add_argument("--pre_epochs", type=float, default=180)
    ...
    while epoch <= args.pre_epochs:
        loss,loss1,loss2 = pretrain_epoch(model = MOmoco, 
                                          data = g_feats, 
                                          neighbors = neighbors, 
