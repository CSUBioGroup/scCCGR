...
class GetData(Dataset):
    def __init__(self, dataset_name):
        ...
        ADT_matrix_path = 'data/' + dataset_name + '/Processed_dataset/ADT/matrix.mtx'
        ...
        ADT_matrix = sc.read_mtx(ADT_matrix_path)
        ...
        ADT_matrix = normalizeADT(ADT_matrix)

        ATAC_neighbors, _ = cal_nn(ATAC_matrix,k=6)
        RNA_neighbors, _ = cal_nn(RNA_matrix,k=6)
        ADT_neighbors, _ = cal_nn(ADT_matrix, k=6)

        self.g_feats = [ATAC_matrix, RNA_matrix,ADT_matrix]
        self.neighbors = [ATAC_neighbors, RNA_neighbors,ADT_neighbors]
...
def load_data(dataset_name):
    ...
    dims = [dataset.V1.shape[1], dataset.V2.shape[1], dataset.V3.shape[1]]
    view = 3
...
def load_moco_data(dataset_name):
    ...
    adt_adj_n = adj_normalize(adt_adj)
    wnn_adj_n = adj_normalize(wnn_adj)

    adjs = [atac_adj.toarray(), rna_adj.toarray(), adt_adj.toarray(), wnn_adj.toarray()]
    adjs_n = [atac_adj_n, rna_adj_n, adt_adj_n, wnn_adj_n]
    g_feats = [z_atac, z_rna, z_adt]
